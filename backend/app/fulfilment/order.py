...
async def apply_fulfillment_defaults(
...
async def validate_and_move_to_querying_if_needed(
    *,
    ...
    if not validation_succeeded:
        template_id = await get_product_template_id(ext_client, QUERYING_TEMPLATE_TYPE, None)
...
async def fulfill_order(
    *,
    ...
    order, employee_id, employee_email = await create_employee(
    ...
    template_id = await get_product_template_id(ext_client, COMPLETED_TEMPLATE_TYPE, template_name)
    ...
    if is_new:
        try:
            await optscale_client.reset_password(employee_email)
